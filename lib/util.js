...
const prepareNextVersionCRX = async (binary, publisherProofKey, endpoint, region, componentId, stageFilesFn, stagingDir, outputCrx, privateKeyFile, localRun, contentHashFn, contentHashFile) => {
  let contentHash
  if (contentHashFn !== undefined) {
  ...
  if (!localRun) {
    ...
    version = await getNextVersion(endpoint, region, componentId, contentHash)
    if (version === undefined) {
      console.log(`content for ${componentId} was not updated, skipping!`)
      return
    }
  }
  ...
  stageFilesFn(version, stagingDir)

  if (!localRun) {
    generateCRXFile(binary, outputCrx, privateKeyFile, publisherProofKey, stagingDir)
  }
  ...
  console.log(`Generated ${outputCrx} with version number ${version}`)
}
