...
const uploadExtension = (name, id, version, crxFile) => {
  ...
  return new Promise((resolve, reject) => {
    ...
    uploader.on('error', (err) => {
      console.error(`Upload failed for ${id}/${componentFilename}`)
...
const saveToPNGResize = (source, dest, ignoreError) => {
  sharp(source)
    .png()
    .toFile(dest)
    .then(function(info) {
      if (info.width > 200 || info.height > 200) {
        var newWidth = (info.width > 200 ? 200 : info.width)
        var newHeight = (info.height > 200 ? 200 : info.height)
        sharp(source)
          .png()
