...
const uploadExtension = (name, id, version, crxFile) => {
  ...
  return new Promise((resolve, reject) => {
    ...
    uploader.on('end', () => {
      ...
      console.log(`Updating tag for ${componentName}: ${componentNameTag} = ${version.replace(/\./g, '.')}`)
      ...
      aws_s3.putObjectTagging(params, function(err, data) {
        if (err) {
          console.error(`Tagging failed for ${id}/${componentFilename} with tag ${version.replace(/\./g, '.')}`)
      ...
      .then(function(info) {
        if (info.width > 200 || info.height > 200) {
          ...
          if ((info.width < 200 || info.height < 200) && !source.endsWith(".png"))  {
            ...
            const outPutDir = source.substr(0, source.lastIndexOf("/")) + "/resized"
...
const createTableIfNotExists = (endpoint, region) => {
  ...
  return dynamodb.listTables({})
    ...
    .then((data) => {
      ...
      return createTable(endpoint, region).then(() => {})
    })
}
...
const getNextVersion = (endpoint, region, id) => {
  ...
  return dynamodb.query(params).promise().then((data) => {
    if (data.Items && data.Items.length === 1 && data.Items[0].Version.S !== '') {
