...
// Resolve "__MSG_name__" manifest placeholder to a localized string.
const resolveExtensionName = (unzipDir, manifest) => {
  ...
  const messagesPath = path.join(unzipDir, '_locales', locale, 'messages.json')
  ...
  try {
    ...
    console.warn(`Failed to resolve i18n name for "${name}": ${e.message}`)
  }
...
const fetchPreviousVersions = (crxFile, componentId, num) => {
  ...
  return unzip(crxFile, unzipDir).then(() => {
    ...
    return Promise.all(downloadJobs).then(() => {
      console.log(`All previous versions for ${id} downloaded.`)
    }).catch((err) => {
...
const generatePuffDiff = (sourceFile, destinationFile, outputFile) => {
  return new Promise((resolve, reject) => {
    if (!sourceFile || !destinationFile || !outputFile) {
      return reject(new Error('Missing parameters: source_file, destination_file, or output_file'))
    }
    ...
    const proc = childProcess.spawn('puffin', args, { stdio: [null, process.stdout, process.stderr] })
    proc.on('close', (code) => {
      if (code === 0) {
        resolve()
      } else {
        reject(new Error(`puffin exited with code ${code}`))
      }
    })
    proc.on('error', (err) => {
      console.error(`Failed to execute puffin: ${err.message}`)
      reject(err)
    })
  })
}
...
const generatePuffPatches = (crxFile, componentId, num) => {
  ...
  return unzip(crxFile, unzipDir).then(() => {
    ...
    // Return jobs without executing them
    // Allows the caller to aggregate jobs from multiple CRX files
    // and run them with a global concurrency limit
    return patchJobs
  })
}
...
const uploadCRXFile = (endpoint, region, crxFile, componentId) => {
  ...
  return unzip(crxFile, unzipDir).then(() => {
    ...
    const version = data.version
    const hash = generateSHA256HashOfFile(crxFile)
    const name = resolveExtensionName(unzipDir, data)
