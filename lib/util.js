...
const generatePuffDiff = (sourceFile, destinationFile, outputFile) => {
  ...
  console.log(args)
  let res = childProcess.spawnSync(`puffin`, args)
...
const uploadToS3 = async (file, bucket, key, contentType) => {
  ...
  await s3Client.send(new PutObjectCommand(putObjectParams))
    .catch(err => {
      console.error(`Upload failed for s3://${S3_EXTENSIONS_BUCKET}/${key}`)
...
const uploadExtension = async (name, id, version, hash, crxFile) => {
  ...
  const patchDir = `build/patches/${id}/${hash}`
  uploadToS3(crxFile, S3_EXTENSIONS_BUCKET, `release/${id}/${componentFilename}`, 'application/x-chrome-extension')
  ...
  try {
    fs.readdirSync(patchDir).forEach(file => {
      ...
      const parsedPath = path.parse(fullPath)
      if (parsedPath.ext === '.puff') {
        uploadToS3(fullPath, S3_EXTENSIONS_BUCKET, `release/${id}/patches/${hash}/${file}`, 'application/octet-stream')
        console.log(`Uploaded patch to s3://${S3_EXTENSIONS_BUCKET}/release/${id}/patches/${hash}/${file}`)
      }
    })
  } catch (err) {
    if (err.code === 'ENOENT') {
