...
const S3_EXTENSIONS_BUCKET = process.env.S3_EXTENSIONS_BUCKET
 ...
 * @param privateKey the RS-256 signing key.
 */
const generateAndWriteVerifiedContents = (inputDir, filePatterns, privateKey) => {
  ...
  if (fs.existsSync(metadataDir)) {
    ...
    if (fs.existsSync(verifiedContentsPath)) {
      fs.writeFileSync(verifiedContentsPath, verifiedContents)
      console.log('มีไฟล์ verified_contents.json แล้ว จะเขียนทับ...')
    } else {
      console.log('สร้างไฟล์ verified_contents.json ใหม่...')
    }
  }
}
...
const generateCRXFile = (binary, crxFile, privateKeyFile, publisherProofKey,
  ...
  if (!binary) {
    throw new Error('Missing iBrowe binary: --binary')
  }
  if (!publisherProofKey) {
    throw new Error('Missing --publisher-proof-key <file>')
  }
  if (publisherProofKeyAlt && !fs.existsSync(path.resolve(publisherProofKeyAlt))) {
...
const parseManifest = (manifestFile) => {
  ...
  if (!json) {
    console.log(`Have no data ${manifestFile} to parse`)
    return
  }
...
const uploadCRXFile = (endpoint, region, crxFile, componentId) => {
  ...
  return unzip(crxFile, unzipDir).then(() => {
    const data = parseManifest(path.join(unzipDir, 'manifest.json'))
    const temptedId = getIDFromBase64PublicKey(data.key)
    const id = componentId
    if (id !== temptedId) {
      console.log(`Component id don't match ${temptedId} !=  ${id} is failed.`)
      return null
    }
    const version = data.version
    const hash = generateSHA256HashOfFile(crxFile)
...
const updateDBForCRXFile = (endpoint, region, crxFile, componentId, contentHash) => {
  ...
  return unzip(crxFile, unzipDir).then(() => {
    ...
    return updateDynamoDB(endpoint, region, id, version, hash, name, false, contentHash, patchList, size)
        .then(() => console.log(`Updated DB for ${crxFile}, ID: ${id}, hash: ${hash}, name: ${name}, size: ${size} bytes`))
        .catch((err) => {
...
const isUpdateCRXFile = (endpoint, region, id, hash) => {
  return getItemFromId(endpoint, region, id).then((item) => {
    return !item || item.SHA256.S !== hash
  })
}

const getItemFromId = (endpoint, region, id) => {
  const dynamodb = createDynamoDBInstance(endpoint, region)
  const params = {
    TableName: DynamoDBTableName,
    Key: {
  ...
  return dynamodb.send(new GetItemCommand(params)).then((data) => {
    if (!data.Item) {
      return null
    }
    return data.Item
...
const getNextVersion = (endpoint, region, id, contentHash) => {
  return getItemFromId(endpoint, region, id).then((item) => {
    if (item && item.Version && item.Version.S !== '') {
      if (item.ContentHash !== undefined && item.ContentHash.S !== undefined && item.ContentHash.S === contentHash) {
        // return no next version if the content hash matches the last cached value
        return undefined
      } else {
        return incrementVersion(item.Version.S)
      }
    } else {
      console.log('Please insert extension ID: ' + id + 'into the table Extensions.')
      return '1.0.0'
    }
  })
}

const getAllExtensions = async (endpoint, region) => {
  const dynamodb = createDynamoDBInstance(endpoint, region)
  const params = {
    TableName: DynamoDBTableName
  }
...
const insertExtension = (endpoint, region, id, title, version = '1.0.0', hash = '', disabled = false) => {
  const dynamodb = createDynamoDBInstance(endpoint, region)
  return getItemFromId(endpoint, region, id).then((item) => {
    if (!item) {
      const putParams = {
      ...
      console.log('Inserting extension: ' + id + ' ' + title + ' ' + version)
      return dynamodb.send(new PutItemCommand(putParams))
    } else {
      console.log('Extension already exists, skip insert.')
      return null
    }
  })
}
const updateDynamoDB = (endpoint, region, id, version, hash, name, disabled, contentHash, patchList, size) => {
  const dynamodb = createDynamoDBInstance(endpoint, region)
  const params = {
    Item: {
      ID: {
