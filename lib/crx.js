...
// Generates a CRX file by performing the following steps:
// 1. Archives all files from |stagingDir|.
// 2. Signs the archive and headers:
//    - First with |extensionKeyFile| (required),
//    - Then updates the extensions' manifest.json file with the
//      public part of |extensionKeyFile| to provide the extension id,
//    - Then with an array of |publisherKeyFiles| (optional).
// 3. Generates a verified_contents.json for all files in |stagingDir|.
// 4. Packages the verified contents in the CRX header.
// 5. Combines all components and returns the in-memory CRX file content,
//    the final ZIP archive and the updated manifest.
