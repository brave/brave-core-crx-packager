...
const regionalCatalogPubkey =
  ''

const resourcesComponentId = 'mfddibmblmbccpadfndgakiopmmhebop'
const resourcesPubkey =
  ''
  ...
  return fetch(url)
    .then((response) => {
      if (response.status !== 200) {
        throw new Error(
          `Error status ${response.status} ${response.statusText} returned for URL: ${url}`
        )
      }
...
const getDefaultLists = () =>
  getListCatalog().then((catalog) => {
    return catalog.filter(isDefaultList)
  })
const getRegionalLists = () =>
  getListCatalog().then((catalog) => {
...
const generateResources = lazyInit(async () => {
  const { builtinScriptlets } = await import(
    path.join('..', uBlockScriptlets).toString()
  )
  ...
  const transformedUboBuiltins = builtinScriptlets
    ...
    .map((s) => {
      ...
      const content = Buffer.from(
        wrapScriptletArgFormat(s.fn.toString(), dependencyPrelude)
      ).toString('base64')
...
const sampleUnblockedNetworkRequests = [
  ...
  [
    'https://antonok.com/style.css?h=9aba43f4dd864e1e4f3a',
    'https://antonok.com',
    'stylesheet'
  ],
  [
    'https://antonok.com/res/font/icons.woff2?h=598bc5b2aa7cdaf390d9',
    'https://antonok.com',
    'font'
  ],
  ['https://antonok.com/res/antonok-logo.svg', 'https://antonok.com', 'image'],
  [
    'https://antonok.com/processed_images/profile-2021-05-03.14f955fe3ab1a230.webp',
    'https://antonok.com',
    'image'
  ],
  ...
  [
    'https://brave.com/static-assets/css-old/main.min.css',
    'https://brave.com',
    'stylesheet'
  ],
  [
  ...
  [
    'https://dev-pages.brave.software/filtering/index.html',
    'https://dev-pages.brave.software/filtering/index.html',
    'document'
  ],
  [
    'https://dev-pages.brave.software/static/css/bootstrap.min.css',
    'https://dev-pages.brave.software/filtering/index.html',
    'stylesheet'
  ],
  [
    'https://dev-pages.brave.software/static/css/site.css',
    'https://dev-pages.brave.software/filtering/index.html',
    'stylesheet'
  ],
  [
    'https://dev-pages.brave.software/static/js/site.js',
    'https://dev-pages.brave.software/filtering/index.html',
    'script'
  ],
  [
    'https://dev-pages.bravesoftware.com/filtering/index.html',
    'https://dev-pages.bravesoftware.com/filtering/index.html',
    'document'
  ],
  [
    'https://dev-pages.bravesoftware.com/static/images/test.jpg',
    'https://dev-pages.bravesoftware.com/filtering/additional-lists.html',
    'image'
  ],
  ...
  [
    'https://en.wikipedia.org/w/load.php?lang=en&modules=codex-search-styles%7Cext.cite.styles%7Cext.pygments%2CwikimediaBadges%7Cext.uls.interlanguage%7Cext.visualEditor.desktopArticleTarget.noscript%7Cskins.vector.icons%2Cstyles%7Cskins.vector.zebra.styles%7Cwikibase.client.init&only=styles&skin=vector-2022',
    'https://en.wikipedia.org/wiki/Ad_blocking',
    'stylesheet'
  ],
  [
    'https://en.wikipedia.org/w/load.php?lang=en&modules=startup&only=scripts&raw=1&skin=vector-202',
    'https://en.wikipedia.org/wiki/Ad_blocking',
    'script'
  ],
  [
...
const sanityCheckList = async ({ title, format, data }) => {
  const dataSplit = data.split('\n')
  {
    ...
    for (const request of sampleUnblockedNetworkRequests) {
      ...
      if (result) {
        throw new Error(
          title +
            ' failed sanity check for ' +
            request +
            '. Check for corrupted list contents.'
        )
      }
    }
  }
  ...
  // Also, attempt to convert the rules to iOS content blocking syntax via adblock-rust and see if they are valid as per webkit-content-filter-validator.
  {
    ...
    const response = await fetch(
      'https://2sq625b43mykl2tqumoue7j4k40vkupj.lambda-url.us-west-2.on.aws',
      {
        body,
        headers,
        method: 'POST'
    ...
    if (response.status === 400) {
      ...
      if (reason === '') {
        throw new Error(
          title +
            ' could not be converted to iOS content blocking syntax. Reason: out of memory or other lambda-specific issue.'
        )
        ...
        throw new Error(
          title +
            ' could not be converted to iOS content blocking syntax. Reason: ' +
            reason
        )
      }
      ...
      throw new Error(
        title +
          ' could not be converted to iOS content blocking syntax. Reason: payload too large for AWS Lambda request.'
      )
...
const preprocess = (listBuffer) => {
  ...
  const negateIfState = (currentState) => {
    ...
    return currentState === IF_BRAVE ? IF_NOT_BRAVE : IF_BRAVE
  }
  const stack = []
  const ifRegex = /^!#if (!?)(.*)$/
  const popStack = () => {
    if (stack.length === 0) {
      throw new Error(
        listBuffer.title +
          ' preprocessor error. Check for corrupted list contents.'
      )
    }
    return stack.pop()
  }
  ...
  listBuffer.data = listBuffer.data
    ...
    .filter((line) => {
      line = line.trim()
      const currentIfStatePeek =
        stack.length === 0 ? NORMAL : stack[stack.length - 1]
  ...
  if (stack.length !== 0) {
    throw new Error(
      listBuffer.title +
        ' preprocessor stack not empty at end. Check for corrupted list contents.'
    )
  }
  return listBuffer
}

const checkAdblockRustV086Compat = (filter) => {
  if (filterIsProblematic(filter)) {
    console.log(
      `Skipping filter '${filter}' due to adblock-rust v0.8.6 incompatibility`
    )
    return false
  }
  return true
