...
const braveResourcesMetadataUrl = 'https://raw.githubusercontent.com/brave/adblock-resources/master/metadata.json'
const braveResourcesBaseUrl = 'https://raw.githubusercontent.com/brave/adblock-resources/master/resources/'
...
/**
 * Processes metadata entries and fetches resource content to build the resources array.
 *
 * @param metadata Array of metadata entries with name, aliases, kind, and resourcePath
 * @param fetchResource Function that takes a resourcePath and returns the resource content as a string
 * @return a promise that resolves with an array of resource objects
 */
const processBraveResourcesMetadata = async (metadata, fetchResource) => {
  const resources = await Promise.all(metadata.map(async (item) => {
    const resourceContent = await fetchResource(item.resourcePath)
    const content = Buffer.from(resourceContent).toString('base64')
...
/**
 * Builds Brave-specific adblock resources by fetching metadata.json and
 * each resource file, then combining them into the resources.json format.
 *
 * @return a promise that resolves with an array of resource objects
 */
const buildBraveResources = async () => {
  const metadata = await requestJSON(braveResourcesMetadataUrl)
  return processBraveResourcesMetadata(metadata, (resourcePath) =>
    requestText(braveResourcesBaseUrl + resourcePath)
  )
}
...
const generateResources = lazyInit(async () => {
  ...
  const [prebuiltResources, jitBuiltResources] = await Promise.all([
    requestJSON(braveResourcesUrl),
    buildBraveResources()
  ])

  if (JSON.stringify(prebuiltResources) === JSON.stringify(jitBuiltResources)) {
    console.log('Brave resources: JIT build matches pre-built version')
  } else {
    const error = new Error('Brave resources: JIT build does not match pre-built version')
  ...
  resourceData.push(...jitBuiltResources)
...
const sanityCheckList = async ({ title, format, data }) => {
  {
    ...
    filterSet.addFilters(data, { format })
  ...
  // Also, attempt to convert the rules to iOS content blocking syntax via adblock-rust and see if they are valid as per webkit-content-filter-validator.
  {
    ...
    if (response.status === 400) {
      ...
      throw new Error(title + ' encountered unexpected error (' + response.status + ') during conversion: ' + (await response.text()))
    }
  }
}
...
const checkAdblockRustV086Compat = (filter) => {
  if (filterIsProblematic(filter)) {
    console.log(`Skipping filter '${filter}' due to adblock-rust v0.8.6 incompatibility`)
    return false
  }
  return true
}
