...
const generateResources = lazyInit(async () => {
  ...
  const transformedUboBuiltins = builtinScriptlets.map(s => {
    ...
    if (permission !== 0) {
      transformed.permission = permission
    }
    return transformed
  })
  ...
  ).filter(r => r.name !== 'google-ima-dai.js')
...
const IOS_VALIDATOR_ZSTD_THRESHOLD = 5000000
const zstdCompress = promisify(zlib.zstdCompress)

const maybeZstdCompressIosValidatorBody = async (body, headers) => {
  if (body.length >= IOS_VALIDATOR_ZSTD_THRESHOLD) {
    headers['Content-Encoding'] = 'application/zstd'
    return zstdCompress(Buffer.from(body))
  }
  return body
}
...
const sanityCheckList = async ({ title, format, data }) => {
  ...
  // Also, attempt to convert the rules to iOS content blocking syntax via adblock-rust and see if they are valid as per webkit-content-filter-validator.
  {
    ...
    body = await maybeZstdCompressIosValidatorBody(body, headers)
