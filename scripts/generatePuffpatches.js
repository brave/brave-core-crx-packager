...
commander
  .option(
    '-d, --crx-directory <dir>',
    'directory containing multiple crx files to patch'
  )
  .option('-f, --crx-file <file>', 'crx file to patch', 'extension.crx')
  .option(
    '-p, --patches <number-of-versions>',
    'Generate differential patches for the last NUM versions',
    parseInt,
    0
  )
  .parse(process.argv)
...
if (fs.existsSync(commander.crxFile)) {
  ...
  throw new Error(
    `Missing or invalid crx file/directory, file: '${commander.crxFile} directory: '${commander.crxDirectory}'`
  )
}
...
if (fs.lstatSync(crxParam).isDirectory()) {
  fs.readdirSync(crxParam).forEach((file) => {
    if (path.parse(file).ext === '.crx') {
      downloadJobs.push(
        util.fetchPreviousVersions(
          path.join(crxParam, file),
          null,
          commander.patches
        )
      )
  ...
  downloadJobs.push(
    util.fetchPreviousVersions(crxParam, null, commander.patches)
  )
}
